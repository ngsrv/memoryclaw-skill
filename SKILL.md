...
**Last line of defense — CLI bypass nudge (v1.2.1+):** even when no host-tool hooks are available (Aider, Goose, generic AGENTS.md, OR Claude Code where the user hasn't installed the plugin), the CLI itself prints a `⚠ MemoryClaw bypass nudge: engramming without a recent recall …` warning to stderr whenever `memoryclaw memory engram` is called without a `memoryclaw memory recall` in the last 30 minutes. Stderr lands in the agent's tool output, so the warning is impossible to silently miss the way SKILL.md text is. Configurable via `MEMORYCLAW_BYPASS_NUDGE`: `off` (silence — CI-friendly), `warn` (default — print + continue), or `strict` (print + exit non-zero, forcing the agent to recall). If you see this warning in your stderr, run a `memoryclaw memory recall "<3-8 keywords>"` BEFORE your next engram — past sessions on this account may already contain the fact you're about to save.
